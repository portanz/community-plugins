...
- **Recent package updates.** A crash that starts right after an update points at
  the update (on Arch: `/var/log/pacman.log`).
...
Most distributions run a debuginfod server (Arch:
`https://debuginfod.archlinux.org`, others: see `debuginfod.<distro>` docs):
...
If this program keeps crashing and the user has heard enough about it, they can
mute its notifications from the Crashes panel (the bell button on its row) —
mention that, but never mute anything yourself.
